...
def calculate_daily_revenue(df, num_txns):
  return num_txns * (df.apply(
      lambda row: row['med_l2_rev'] * row['pct_distribution'], axis=1).sum())


def calculate_daily_cost(df, med_gas_price):
  df['daily_l1_gas_used_inbox'] = df.apply(
      lambda row: regression_model(row['med_calldata_bytes'], row[
          'med_l1_gas_used'], num_txns * row['pct_distribution']),
      axis=1)
  return (med_gas_price * df['daily_l1_gas_used_inbox'].sum()) / 1e9
...
if run:
  try:
    past24h_txn_data = get24hTxnData()
  ...
  st.dataframe(past24h_txn_data)

  st.subheader("Step 2: Assume % distribution of txn types")
  set_pct_distribution(past24h_txn_data)
  ...
  st.caption(
      "Sum of (txn revenue * number of transactions) for each transaction type"
  )
  daily_rev = calculate_daily_revenue(past24h_txn_data, num_txns)
  st.write(f"Predicted Daily Revenue = {round(daily_rev, 3)} ETH")

  st.subheader("Step 4: Predict daily cost for the rollup")
  st.caption(
      "Regression Model is used to calculate the Gas used on L1. This is multiplied by the median gas price of the past day."
  )
  med_gas_price = getMedGas()['median_gas_price_gwei'][0]
  daily_cost = calculate_daily_cost(past24h_txn_data, med_gas_price)
  st.write(f"Predicted Daily Cost = {round(daily_cost, 3)} ETH")

  st.subheader('Output', divider='rainbow')
  st.caption(
      "Predicted Daily Profit = Predicted Daily Revenue - Predicted Daily Cost"
  )
  daily_profit = daily_rev - daily_cost
  st.subheader(f"Predicted Daily Profit = {round(daily_profit, 2)} ETH")
